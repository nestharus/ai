...
`phase3-rebind` advances only Phase 3 binding attempt 1 to 2 or 2 to 3. It requires byte-distinct attempt-versioned proposal and estimate artifacts, exact retained prior estimate/proposal identities, an exact blocked Phase 4 return decision and audit identity, complete prior revision lineage, and exactly five changed manifest keys: the estimate ref/hash, binding attempt, revision history, and phase history. Replay, skip, reorder, diversion, stale evidence, and attempt 4 fail closed. Like the other pre-PR binds, it journals the complete active index and artifact set as read-only guards and writes only `session.json`.
...
- `3`: provider/git capture failure, stale apply input/source identity, transaction failure, or recovery failure.
