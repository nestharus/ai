...
For both ticket backends, `producer_log_path` carries the closed `ticket-operation-producer-log-v1` operation-identity projection and `producer_output_path` carries the `ticket-operation-readback-v1` projection. The operator translates verified client responses into those exact field sets. `validate-ticket-operation-result` checks their current hashes, declared identities and caller-context consistency. Its `VALID` result does not prove a complete request/response sequence, request order, native final phase or remote execution. Any underlying client/runner transcript belongs to the invoking workflow's execution/evidence capture policy and is separate from these projections and from process-lineage authentication; this schema does not guarantee its capture.
